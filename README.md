...
下载 Release 中的 `ThemeXmlTips-Setup-2.1.0.exe` 双击运行：
...
1. 下载 Release 中的 `theme-xml-tips-2.1.0.vsix`
2. VS Code → 扩展面板 → `...` → `Install from VSIX...` → 选择该文件

或命令行：

```bash
code --install-extension theme-xml-tips-2.1.0.vsix
```

**WebStorm（Live Templates）**

1. 下载 Release 中的 `ThemeXmlTips-WebStorm-2.1.0.zip`
2. `File` → `Manage IDE Settings` → `Import Settings...` → 选择该 zip，重启后生效
3. 在 `.xml` 文件中输入唤醒词（如 `var`、`image-view`、`unlock`）即可展开模板

**HBuilderX（自定义代码块）**

1. 下载 Release 中的 `ThemeXmlTips-HBuilderX-2.1.0.zip` 并解压
2. `工具` → `自定义代码块` → 打开 `xml.json`，将压缩包内 `xml.json` 的内容合并进去保存
3. 在 `.xml` 文件中输入唤醒词即可唤出代码块

**Sublime Text（Snippets）**

1. 下载 Release 中的 `ThemeXmlTips-Sublime-Text-2.1.0.zip` 并解压
